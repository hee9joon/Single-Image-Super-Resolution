...
class DIV2K(Dataset):
    def __init__(self, sort, original_size, crop_size, upscale_factor, patch_size, patch, flip):
    ...
    def __getitem__(self, index):
        ...
        if self.sort == 'train':
            ...
            img_gt = hr_image.transpose(2, 0, 1).astype(np.float32)
            img_lr = lr_image.transpose(2, 0, 1).astype(np.float32)
...
def get_div2k_loader(sort, batch_size, image_size, upscale_factor, crop_size, patch_size=None, patch=False, flip=False):
    """Prepare DIV2K Loader"""

    if sort == 'train':
        dataset = DIV2K(sort, image_size, crop_size, upscale_factor, patch_size, patch, flip)
